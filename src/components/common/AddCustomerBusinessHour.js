...
function AddCustomerBusinessHour({ history }) {
...
const handleSubmit = async (e) => {
  ...
  try {
    await toast.promise(
      api.post(`products/addproducts/${selectedCustomer._id}`, formData, {
  ...
  return (
    <>
                ...
                <TableCell>
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      layout="outline"
                      size="small"
                      onClick={() => history.push(`/app/all-customer-products/${customer._id}`)}
                    >
                      All Products
                    </Button>
                  </div>
      ...
      >
        <h2 className="text-xl font-bold mb-4">Add Product for {selectedCustomer?.name}</h2>
...
export default AddCustomerBusinessHour;
